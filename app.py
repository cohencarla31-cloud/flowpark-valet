...
# --- CARGA DE DATOS (TARIFAS Y EXTRAS SEPARADOS) ---
def obtener_datos():
    tarifas_default = {
...
if menu == "📥 Ingreso":
    ...
    nombre_cli = st.text_input("Nombre Cliente:", value=nombre)
    cel_cli = st.text_input("Celular del cliente:", value=celular)
    ...
    if st.button("Calcular y Generar Salida"):
        if tarjeta_salida:
            try:
                ...
                fila_idx, fila_ingreso = None, None
                for idx, r in enumerate(reg, start=2):
                    if str(r.get("Ticket", "")).strip().lstrip("0") == t_clean and not str(r.get("Hora_Salida", "")).strip() and str(r.get("Ticket")) != "EXTRA":
                        fila_idx = idx
                        fila_ingreso = r
                        break
                ...
                if not fila_ingreso:
                    ...
                    patente = fila_ingreso.get("Matricula", fila_ingreso.get("Matrícula"))
                    hora_ing = datetime.strptime(fila_ingreso.get("Hora_Ingreso"), "%Y-%m-%d %H:%M:%S")
                    minutos = int((datetime.utcnow() - timedelta(hours=3) - hora_ing).total_seconds() / 60)
                    
                    # Verificación Quinquela local
                    try:
                        quinquela_data = sh.worksheet("Respuestas de formulario 1").get_all_records()
                        tiene_q = any(str(q.get("PATENTE", "")).upper().replace("-","").replace(" ","") == patente.upper().replace("-","").replace(" ","") for q in quinquela_data)
                    except:
                        tiene_q = False
                    
                    # Cálculo automático de estadía
                    es_camioneta = "Camioneta" in fila_ingreso.get("Estado", "")
                    monto_estacionamiento = calcular_mejor_precio(minutos, es_camioneta, tiene_q)
                    
                    # Sumar extras pendientes de este auto
                    extras_auto = [r for r in reg if str(r.get("Ticket", "")).upper() == "EXTRA" and str(r.get("Matrícula", r.get("Matricula", ""))).upper() == patente.upper() and not str(r.get("Hora_Salida", "")).strip()]
                    ...
                    info_desc = "Incluye 2h libres de Quinquela." if tiene_q else "Tarifa optimizada aplicada."
                    
                    # Generar Ticket de Egreso
                    texto_ticket = f"""*FLOW PARK - TICKET DE EGRESO*
---------------------------------
🚗 Vehículo: {patente} | Tarjeta: #{tarjeta_salida}
⏱️ Estadía: {minutos//60}h {minutos%60}m
---------------------------------
📋 DETALLE:
{detalle_extras if detalle_extras else "Sin extras."}
...
ℹ️ {info_desc}
---------------------------------
¡Gracias por elegirnos! Buen viaje.
"""
                    ...
                    sh.worksheet("Registro").update_cell(fila_idx, 4, hora_actual_uy())
                    for idx_ex, r_ex in enumerate(reg, start=2):
                        if str(r_ex.get("Ticket", "")).upper() == "EXTRA" and str(r_ex.get("Matrícula", r_ex.get("Matricula", ""))).upper() == patente.upper() and not str(r_ex.get("Hora_Salida", "")).strip():
                            sh.worksheet("Registro").update_cell(idx_ex, 4, hora_actual_uy())
