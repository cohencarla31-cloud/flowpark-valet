...
def init_connections():
    ...
    # 1. Planilla principal (Obligatoria)
    try:
        sh_valet = client.open("FlowPark_Valet_DB")
    except Exception as e:
        st.error(f"❌ Error al abrir la planilla principal 'FlowPark_Valet_DB': {e}")
        sh_valet = None

    # 2. Planilla Quinquela (Protegida contra errores 404)
    try:
...
# Cargar empleados de forma segura
...
# Cargar Tarifas y Extras de forma segura
...
if not sh:
    st.error("⚠️ La aplicación no pudo conectarse a la base de datos principal. Verifica el nombre 'FlowPark_Valet_DB' y los permisos de la cuenta de servicio en Google Drive.")
    st.stop()
